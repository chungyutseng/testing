...
count = 0
pxs = np.zeros((2,), dtype=np.float32)
pzs = np.zeros((2,), dtype=np.float32)
pyaws = np.zeros((2,), dtype=np.float32)
...
def get_marker_message(marker_msg):
    ...
    if count == 1:
        ...
        count = count + 1
    if count >= 2:
        # tello_pose_marker_lp[0] = (1.0 / 3.0) * (temp[0] + pxs[1] + pxs[0])
        # tello_pose_marker_lp[2] = (1.0 / 3.0) * (temp[2] + pxs[1] + pxs[0])
        tello_pose_marker_lp[4] = (1.0 / 3.0) * (temp[4] + pxs[1] + pxs[0])

...
rospy.init_node('pd_controller_stage_three_drone2', anonymous=True)
...
pub_vel = rospy.Publisher("/drone2/tello/cmd_vel", Twist, queue_size=10)
...
def get_stage_three_pdc_on(data):
    global stage_three_pdc_on
    stage_three_pdc_on = data.data

def get_estimated_velocity(data):
    global estimated_velocity
    estimated_velocity = data.data
...
rospy.Subscriber('/drone2/tello_pose_kf', numpy_msg(Floats), callback=get_kf_position)
rospy.Subscriber('/drone2/desired_pose', numpy_msg(Floats), callback=get_desired_pose)
rospy.Subscriber('/drone2/estimated_velocity_new', Float32, callback=get_estimated_velocity)
rospy.Subscriber('/drone2/stage_three_pdc_on', Float32, callback=get_stage_three_pdc_on)

while not rospy.is_shutdown():
    pd_controller(current_pose, desired_pose, kp, kd)
    if (stage_three_pdc_on == 1.0):
